...
from argo_automation_cpas import webapi
...
async def test_probe_success():
    ...
    await webapi.probe(session, "https://api.example.com")
...
async def test_fetch_topology_config_success():
    session = _mock_session("get", json_data={
        "data": [{"type": "GOCDB", "feed_url": "https://gocdb.example.com/feeds"}]
    })

    result = await webapi.fetch_topology_config(session, "/api/v2/feeds/topology")
...
async def test_fetch_topology_config_with_token():
    session = _mock_session("get", json_data={"data": [{"type": "CSV"}]})

    result = await webapi.fetch_topology_config(session, "/api/v2/feeds/topology", token="comp-tok")
...
async def test_fetch_topology_config_empty_data():
...
async def test_fetch_topology_config_connection_error():
    cm = AsyncMock()
    cm.__aenter__ = AsyncMock(side_effect=aiohttp.ClientConnectionError("fail"))
    cm.__aexit__ = AsyncMock(return_value=False)
    session = MagicMock()
...
async def test_refresh_tokens_success(settings):
    response = MagicMock()
    response.json = AsyncMock(return_value={"access_token": "tok-abc"})
    response.raise_for_status = MagicMock()
    ...
    tokens = await webapi.refresh_tokens(session, settings)
...
async def test_refresh_tokens_partial_failure(settings):
    ...
    def make_cm(*args, **kwargs):
        ...
        response = MagicMock()
        response.json = AsyncMock(return_value={"access_token": "tok-sensu"})
        response.raise_for_status = MagicMock()
        cm = AsyncMock()
        cm.__aenter__ = AsyncMock(return_value=response)
        cm.__aexit__ = AsyncMock(return_value=False)
...
@patch("argo_automation_cpas.webapi.fetch_topology_config", new_callable=AsyncMock)
@patch("argo_automation_cpas.webapi.save_tokens")
@patch("argo_automation_cpas.webapi.refresh_tokens", new_callable=AsyncMock)
...
async def test_run_refreshes_and_fetches_config(mock_cs, mock_refresh, mock_save, mock_fetch, settings):
    ...
    mock_refresh.assert_called_once_with(session, settings)
